...
function TierRow({ tierData, isCurrentTier, isBetterTier, index }) {
  ...
  return (
    ...
    >
      {/* Tier Icon & Name */}
      <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-[70px] sm:min-w-[100px]">
        <div className={cn(
          "flex items-center justify-center w-6 h-6 sm:w-8 sm:h-8 rounded-md sm:rounded-lg shrink-0",
          "bg-gradient-to-br",
          config.bgGradient,
          "border border-white/50 dark:border-white/10"
        )}>
          <TierIcon className={cn("w-3 h-3 sm:w-4 sm:h-4", config.color)} />
        </div>
        <div className="flex flex-col min-w-0">
          ...
          {isCurrentTier && (
            <span className="text-[8px] sm:text-[10px] font-medium text-emerald-600 dark:text-emerald-400 flex items-center gap-0.5">
      ...
      {/* Concurrency */}
      <div className="flex items-center gap-1 sm:gap-1.5 min-w-[35px] sm:min-w-[55px]">
        <Zap className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-amber-500" />
        <span className="text-xs sm:text-sm font-mono tabular-nums text-stone-700 dark:text-zinc-300">
          {tierData.concurrency}×
        </span>
      </div>

      {/* Delay */}
      <div className="hidden xs:flex items-center gap-1 sm:gap-1.5 min-w-[45px] sm:min-w-[65px]">
      ...
      {/* Estimated time - hidden on very small screens */}
      ...
      <div className="hidden sm:block min-w-[50px] text-right">
        {isBetterTier && tierData.percentFaster > 0 && (
          <span className={cn(
            "inline-flex items-center px-1.5 sm:px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-semibold",
...
export function SpeedComparison({ gatewayId = 'auth', className, hideHeader = false }) {
  ...
  if (isLoading) {
    return (
      <div className={cn("flex items-center justify-center py-6 sm:py-8", className)}>
        <Loader2 className="w-5 h-5 sm:w-6 sm:h-6 animate-spin text-stone-400 dark:text-zinc-500" />
      </div>
    );
  }
  ...
  return (
    <div className={className}>
      {!hideHeader && (
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-stone-500 dark:text-zinc-500" />
            <span className="text-xs sm:text-sm font-medium text-stone-700 dark:text-zinc-300">
              Speed Comparison
            </span>
          </div>
          {error && (
            <span className="text-[9px] sm:text-[10px] font-medium px-1.5 sm:px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400">
              Using defaults
            </span>
          )}
        </div>
      ...
      {/* Legend */}
      <div className="mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-stone-200/50 dark:border-zinc-700/50 flex flex-col xs:flex-row items-start xs:items-center justify-between gap-1.5 xs:gap-2 text-[9px] sm:text-[10px] text-stone-500 dark:text-zinc-500">
        <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
          <span className="flex items-center gap-1">
            <Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> Concurrent
          </span>
          <span className="hidden xs:flex items-center gap-1">
            <Clock className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> Delay
          </span>
        </div>
        <span className="hidden sm:inline">Est. time for 100 cards</span>
      </div>
    </div>
  );
}
