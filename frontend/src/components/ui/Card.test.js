...
/**
 * **Feature: shadcn-ui-migration, Property 8: Status-based styling applied correctly**
...
// Uses shadcn/tailwind utility classes
const statusStyleMapping = {
  ...
  approved: { borderClass: 'border-l-primary', bgClass: 'bg-primary/10' },
  warning: { borderClass: 'border-l-amber-500', bgClass: 'bg-amber-50/30' },
  info: { borderClass: 'border-l-sky-500', bgClass: 'bg-sky-50/30' },
};
...
function getAppliedStatusStyles(status) {
  const statusStyles = {
    ...
    approved: 'border-l-primary bg-primary/10',
...
/**
 * **Feature: shadcn-ui-migration, Property 9: Selected card shows ring highlight**
...
function getSelectionStyles(isSelected) {
  // Based on ResultCard implementation: primary ring highlight for selected state
  return isSelected 
...
describe('ResultCard Selection State', () => {
   ...
   * include ring highlight CSS classes (ring-2, ring-primary, ring-offset)
   */
  it('should apply ring highlight classes when isSelected is true', () => {
    fc.assert(
      fc.property(fc.constant(true), (isSelected) => {
        ...
        expect(selectionStyles).toContain('ring-primary');
